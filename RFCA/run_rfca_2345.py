"""
This file (rf_ca_2345.py) is designed for:
    RF + CA, generate prediction on 2, 3, 4 and 5 stages.
...
def predict_each_slice(
    ...
    for les_lst in cur_les_lst:
        ...
        driven_2nd, pred_2nd = estimate_ca(
            model, factors, mask, lesions, class_ids, ma, interval[1] - interval[0], half, interval[1] - interval[0],
        )

        factors = cur_factors
        mask = cur_mask
        lesions = cur_lesions
        driven_3rd, pred_3rd = estimate_ca(
            model, factors, mask, lesions, class_ids, ma, interval[2] - interval[1], half, interval[1] - interval[0],
        )
        ...
        mask = cur_mask
        lesions = pred_3rd
        driven_4st, pred_4st = estimate_ca(
            model, factors, mask, lesions, class_ids, ma, interval[2] - interval[1], half, interval[1] - interval[0],
        )
        ...
        mask = cur_mask
        lesions = pred_4st
        driven_5st, pred_5st = estimate_ca(
            model, factors, mask, lesions, class_ids, ma, interval[2] - interval[1], half, interval[1] - interval[0],
        )
        ...
        pred_4st = (pred_4st[..., None] == np.arange(3)) * 1.0
        pred_5st = (pred_5st[..., None] == np.arange(3)) * 1.0
        row2 = np.concatenate(
            (
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--ckp", default="rfca_2345")
